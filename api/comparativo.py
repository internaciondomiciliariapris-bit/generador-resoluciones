...
Genera el CUADRO COMPARATIVO de audifonos, calcado al modelo PRIS.

Soporta 2 a 8 proveedores y devuelve Excel (.xlsx) o PDF.

requirements.txt debe incluir:  openpyxl  y  reportlab
...
POST JSON:
{
  ...
  "cantidad": 2,
  "cotizaciones": [{"nombre": "IAR Argentina", "precio": 1570000}, ...],
  "idx_ganadora": 2,
  "nro_convocados": 4,
...
TEXTO_ADJUDICA = ("CONFORME A LO DETALLADO EN EL CUADRO COMPARATIVO , SE ADJUDICA LA "
                  "COMPRA DE LO SOLICITADO EN EL EXPTE DE REFERENCIA A LA FIRMA :  ")
...
def _lineas_constancia(nro_convocados, firmas):
    return [
        "Se deja constancia que, habiendose solicitado cotizacion a %s proveedores del rubro, unicamente" % nro_convocados,
        "las firmas comerciales: %s; presentaron presupuestos dentro" % firmas,
...
def _leer(d):
    ...
    return {
        ...
        "firmas": str(d.get("firmas_presentaron", "")).strip(),
    }


def pesos_ar(n):
    """1570000 -> '$ 1.570.000,00' (formato argentino)"""
...
def build_xlsx_bytes(d):
    v = _leer(d)
    n = len(v["cots"])
    ultima = get_column_letter(2 + n)          # C=3 -> ultima columna de proveedor
    cols_prov = [get_column_letter(3 + i) for i in range(n)]
    ...
    c = ws["A3"]
    c.value = ("EXPTE : %s - Paciente %s" % (v["expte"], v["paciente"])
               + " " * 60 + "(Adjudicacion de Fecha %s)" % v["fecha_adj"])
    c.font = F_EXPTE
    c.alignment = CTR
    for fila in ws["A3:%s5" % ultima]:
        for cc in fila:
            cc.border = BORDER_ALL

    # Encabezado de tabla
    for col in ["A", "B"] + cols_prov:
        ws.merge_cells("%s6:%s7" % (col, col))
    ws["A6"] = "Detalle";  ws["A6"].font = F_HEAD
    ws["B6"] = "Cantidad"; ws["B6"].font = F_HEAD
    for i, cot in enumerate(v["cots"]):
        cel = ws["%s6" % cols_prov[i]]
        cel.value = cot["nombre"]
        cel.font = F_HEAD_B
        if i == v["gan"]:
    ...
    c = ws["A10"]
    c.value = TEXTO_ADJUDICA + v["cots"][v["gan"]]["nombre"]
    c.font = F_CONF
    c.alignment = LEFT
    for fila in ws["A10:%s13" % ultima]:
        for cc in fila:
            cc.fill = GRIS_CONFORME

    # Constancia
    for k, txt in enumerate(_lineas_constancia(v["convocados"], v["firmas"])):
        cel = ws["A%d" % (14 + k)]
        cel.value = txt
        cel.font = F_CONF
...
def _ajustar(cv, texto, fuente, tam_max, ancho):
    """Baja el tamano de fuente hasta que el texto entre en el ancho dado."""
    t = tam_max
    while t > 5 and cv.stringWidth(texto, fuente, t) > ancho:
        t -= 0.5
    return t
...
def build_pdf_bytes(d):
    ...
    for i in range(n):
        xs.append(xs[2] + w_prov * (i + 1))

    # relleno de la ganadora
    x_gan = xs[2] + w_prov * v["gan"]
    cv.setFillColorRGB(0.816, 0.808, 0.808)
    cv.rect(x_gan, y - h_head - h_data, w_prov, h_head + h_data, stroke=0, fill=1)
...
def _partir(cv, texto, fuente, tam, ancho):
    """Corta el texto en lineas que entren en el ancho."""
    palabras = texto.split()
    lineas, actual = [], ""
    for p in palabras:
        prueba = (actual + " " + p).strip()
        if cv.stringWidth(prueba, fuente, tam) <= ancho:
            actual = prueba
        else:
            if actual:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            formato = str(data.get("formato", "xlsx")).lower()

            if formato == "pdf":
                contenido = build_pdf_bytes(data)
                ctype = "application/pdf"
                fname = nombre_archivo(data, "pdf")
            else:
                contenido = build_xlsx_bytes(data)
                ctype = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                fname = nombre_archivo(data, "xlsx")
