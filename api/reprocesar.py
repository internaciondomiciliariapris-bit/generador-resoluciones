"""
api/reprocesar.py  -  Vercel Python Serverless Function

Reprocesa en lote resoluciones de AUDIFONOS que salieron con el precio
unitario multiplicado x100. Recibe los PDF (firmados o no), lee su texto,
recalcula el unitario = total // cantidad y regenera cada resolucion con el
MISMO formato del original, corrigiendo solo el valor y SIN la leyenda de
firma (se vuelven a firmar). Devuelve un ZIP con todas las corregidas.

POST JSON:
{ "pdfs": [ { "nombre": "Resolucion_4691_GRAMAJO.pdf", "base64": "<PDF b64>" }, ... ] }

Respuesta:
{ "ok": true,
  "zip_base64": "<ZIP en base64>",
  "resultados": [ {"nombre":..., "nroRes":..., "paciente":..., "unitario":..., "total":..., "cantidad":...}, ... ],
  "errores":   [ {"nombre":..., "error":...}, ... ] }

Requiere pypdf (ver requirements.txt). Reusa los logos de pdf.py.
"""
import json, base64, io, zipfile, re, os, tempfile
from http.server import BaseHTTPRequestHandler
from fpdf import FPDF
from pypdf import PdfReader
LOGO1_B64 = ""
LOGO2_B64 = ""


RGB_AZUL = (74, 144, 196)


def _fmt(n):
    return "{:,}".format(int(n)).replace(",", ".")


def _words(segmentos):
    out = []
    for texto, estilo in segmentos:
        for p in texto.split(" "):
            if p == "":
                continue
            out.append((p, estilo))
    return out


def _parrafo(pdf, segmentos, indent=0, size=11, lh=6, x0=25, x1=185, gap_after=3):
    words = _words(segmentos)
    if not words:
        return
    pdf.set_font("Helvetica", "", size)
    space_w = pdf.get_string_width(" ")

    def w_of(word, estilo):
        pdf.set_font("Helvetica", estilo, size)
        return pdf.get_string_width(word)

    lines = []
    cur, cur_w = [], 0.0
    first = True
    avail = x1 - (x0 + indent)
    for word, estilo in words:
        ww = w_of(word, estilo)
        add = ww if not cur else (space_w + ww)
        if cur and cur_w + add > avail + 0.01:
            lines.append((cur, first))
            first = False
            avail = x1 - x0
            cur, cur_w = [(word, estilo, ww)], ww
        else:
            cur.append((word, estilo, ww))
            cur_w += add
    if cur:
        lines.append((cur, first))

    for idx, (line, is_first) in enumerate(lines):
        lx = (x0 + indent) if is_first else x0
        avail = x1 - lx
        is_last = (idx == len(lines) - 1)
        n = len(line)
        words_w = sum(w for _, _, w in line)
        extra = 0.0
        if not (is_last or n == 1):
            extra = (avail - words_w - (n - 1) * space_w) / (n - 1)
            if extra < 0:
                extra = 0.0
        x = lx
        y = pdf.get_y()
        for j, (word, estilo, ww) in enumerate(line):
            pdf.set_font("Helvetica", estilo, size)
            pdf.set_xy(x, y)
            pdf.cell(ww, lh, word)
            x += ww
            if j < n - 1:
                gap = space_w + extra
                pdf.set_font("Helvetica", "", size)
                pdf.set_xy(x, y)
                pdf.cell(gap, lh, " ")
                x += gap
        pdf.set_y(y + lh)
    if gap_after:
        pdf.ln(gap_after)


def construir_resolucion(d, mostrar_firma=False):
    fecha    = d["fecha"]
    nro_res  = d["nroRes"]
    nro_exp  = d["nroExp"]
    paciente = d["paciente"]
    firmas   = d["firmasCons"]
    firma    = d["firmaGan"]
    cantidad = str(d["cantidad"])
    total    = int(d["total"])
    unit     = int(d["unitario"])
    letras   = d["importeLetras"]

    pu = "$ " + _fmt(unit) + ",00"
    pt = "$ " + _fmt(total) + ",00"
    importe_txt = pt + " (" + letras + ")"

    pdf = FPDF(orientation="P", unit="mm", format="A4")
    pdf.set_margins(25, 20, 25)
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=20)

    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as f1:
        f1.write(base64.b64decode(LOGO1_B64)); l1 = f1.name
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as f2:
        f2.write(base64.b64decode(LOGO2_B64)); l2 = f2.name
    pdf.image(l1, x=25, y=15, h=18)
    pdf.image(l2, x=130, y=13, h=20)
    os.unlink(l1); os.unlink(l2)

    pdf.set_draw_color(*RGB_AZUL)
    pdf.set_line_width(0.5)
    pdf.line(25, 36, 185, 36)
    pdf.ln(28)

    pdf.set_font("Helvetica", "", 11)
    pdf.set_x(25)
    pdf.cell(0, 6, f"San Miguel de Tucumán, {fecha}.", align="R")
    pdf.ln(10)

    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 7, f"Resolución Interna: Nº {nro_res}/DGPRIS.", align="C")
    pdf.ln(7)
    pdf.cell(0, 7, "PROGRAMA INTEGRADO DE SALUD", align="C")
    pdf.ln(10)

    pdf.set_font("Helvetica", "BU", 11)
    pdf.set_x(25)
    pdf.cell(0, 6, "VISTO:")
    pdf.ln(8)
    _parrafo(pdf, [
        ("El ", ""),
        (f"Expte. Nº {nro_exp},", "B"),
        (" por el que se solicita la compra de ", ""),
        ("AUDIFONOS", "B"),
        (" para el paciente; ", ""),
        (paciente, "B"),
        (" y", ""),
    ], indent=0, gap_after=4)

    pdf.set_font("Helvetica", "BU", 11)
    pdf.set_x(25)
    pdf.cell(0, 6, "CONSIDERANDO:")
    pdf.ln(8)
    _parrafo(pdf, [("Que se encuentra agregada la documentación correspondiente, la misma esta cumplimentada y debidamente conformada.", "")], indent=20)
    _parrafo(pdf, [
        ("Que se adjunta presupuestos de la Firmas Comerciales; ", ""),
        (f"{firmas}.-", "B"),
    ], indent=20)
    _parrafo(pdf, [
        ("Que la propuesta de ", ""),
        (firma, "B"),
        (" resulto la más conveniente para el Estado.", ""),
    ], indent=20)
    _parrafo(pdf, [("Que la propuesta resulto la más conveniente para el Estado.", "")], indent=20)
    _parrafo(pdf, [("Que se adjunta Dictamen Jurídico con opinión favorable.", "")], indent=20)
    _parrafo(pdf, [("Que se aprueba todo lo actuado por encuadrarse a las disposiciones legales vigentes, que rigen sobre materia de contrataciones, conforme a lo normado en la Res. Nº 388/SPS-05.", "")], indent=20)
    pdf.ln(3)

    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 6, "POR ELLO:", align="C"); pdf.ln(6)
    pdf.cell(0, 6, "LA GERENCIA ADMINISTRATIVA DEL PROGRAMA INTEGRADO DE SALUD", align="C"); pdf.ln(6)
    pdf.cell(0, 6, "RESUELVE:", align="C"); pdf.ln(8)

    _parrafo(pdf, [
        ("ARTICULO", "BU"), (" 1º", "BU"), (" :", "BU"),
        (" Aprobar la compra de Audífonos, a favor de la firma ", ""),
        (firma, "B"),
        (" que se detalla a continuación:", ""),
    ], indent=0, gap_after=2)

    cols = [52, 34, 22, 26, 26]
    x0 = 25
    hdrs = [["FIRMA"], ["INSUMO"], ["CANTIDAD"], ["PRECIO", "UNITARIO"], ["PRECIO", "TOTAL"]]
    hh = 10
    pdf.set_fill_color(*RGB_AZUL); pdf.set_draw_color(*RGB_AZUL)
    pdf.set_text_color(255, 255, 255); pdf.set_font("Helvetica", "B", 9)
    y = pdf.get_y(); x = x0
    for i, ls in enumerate(hdrs):
        pdf.rect(x, y, cols[i], hh, style="DF")
        ty = y + (hh - len(ls) * 4.5) / 2
        for ln_ in ls:
            pdf.set_xy(x, ty); pdf.cell(cols[i], 4.5, ln_, align="C"); ty += 4.5
        x += cols[i]
    pdf.set_xy(x0, y + hh)

    pdf.set_text_color(0, 0, 0); pdf.set_draw_color(0, 0, 0); pdf.set_font("Helvetica", "", 9)
    dh = 11
    y = pdf.get_y(); x = x0
    row = [firma, "Audífonos", cantidad, pu, pt]
    for i, val in enumerate(row):
        pdf.rect(x, y, cols[i], dh)
        if i == 0:
            pdf.set_xy(x, y + 1.5); pdf.multi_cell(cols[i], 4.5, val, align="C")
        else:
            pdf.set_xy(x, y + (dh - 4.5) / 2); pdf.cell(cols[i], 4.5, val, align="C")
        x += cols[i]
    pdf.set_xy(x0, y + dh)

    y = pdf.get_y(); pdf.set_font("Helvetica", "B", 9)
    wtot = sum(cols[:4])
    pdf.rect(x0, y, wtot, 7); pdf.set_xy(x0, y + 1.25); pdf.cell(wtot, 4.5, "TOTAL", align="C")
    pdf.rect(x0 + wtot, y, cols[4], 7); pdf.set_xy(x0 + wtot, y + 1.25); pdf.cell(cols[4], 4.5, pt, align="C")
    pdf.set_xy(x0, y + 7); pdf.ln(6)

    _parrafo(pdf, [
        ("ARTICULO", "BU"), (" 2º", "BU"), (" :", "BU"),
        (" Autorizar la compra al proveedor ", ""),
        (firma, "B"),
        (" , por el importe de ", ""),
        (importe_txt, "B"),
        (" , y posterior emisión de la orden de pago respectiva.", ""),
    ], indent=0, gap_after=2)

    _parrafo(pdf, [
        ("ARTICULO", "BU"), (" 3º:", "BU"),
        (" Imputar la erogación por un importe total de ", ""),
        (importe_txt + ",", "B"),
        (" y posterior emisión de la orden de pago respectiva, a la Jurisdicción 67, Unidad de Organización 965-Recursos disponibles con los que cuenta el servicio-Finalidad/Función 314, Programa 19, Act. /Ob. 01 - Partida 200 - Subpartida 297 - con Financiamiento del Recurso 10 Tesoro General de la Provincia, con cargo al Presupuesto 2026.-", ""),
    ], indent=0, gap_after=2)

    _parrafo(pdf, [
        ("ARTICULO", "BU"), (" 4º:", "BU"),
        (" Pase a control de Ley del Honorable Tribunal de Cuentas.", ""),
    ], indent=0, gap_after=2)

    _parrafo(pdf, [
        ("ARTICULO", "BU"), (" 5º:", "BU"),
        (" Comunicar y archivar. -", ""),
    ], indent=0, gap_after=0)

    if mostrar_firma:
        pdf.ln(16)
        pdf.set_font("Helvetica", "", 10)
        pdf.cell(0, 5, "Firmado digitalmente:", align="C"); pdf.ln(5)
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(0, 5, "C.P.N Mariela Agustina Castillo", align="C"); pdf.ln(5)
        pdf.set_font("Helvetica", "", 10)
        pdf.cell(0, 5, "Gerente Administrativo", align="C"); pdf.ln(5)
        pdf.cell(0, 5, "Dirección Gral. Prog. Integrado de Salud SI.PRO.SA", align="C")

    return bytes(pdf.output())


# ----------------------------- EXTRACCION -----------------------------------
def _norm(t):
    return re.sub(r"\s+", " ", t).strip()


def extraer_datos(pdf_bytes):
    """Lee el texto del PDF y devuelve los campos. Recalcula el unitario a
    prueba de balas como total // cantidad (ignora el unitario roto)."""
    r = PdfReader(io.BytesIO(pdf_bytes))
    t = _norm("\n".join((p.extract_text() or "") for p in r.pages))

    if not re.search(r"AUDIFONOS|Aud[íi]fonos", t):
        raise ValueError("No parece una resolución de audífonos.")

    def buscar(patron, campo):
        m = re.search(patron, t)
        if not m:
            raise ValueError("No se pudo leer el campo: %s" % campo)
        return m

    d = {}
    d["fecha"]      = buscar(r"Tucum[áa]n,\s*(.+?)\.\s*Resoluci", "fecha").group(1).strip()
    d["nroRes"]     = buscar(r"Resoluci[óo]n Interna:\s*N[ºo°]\s*([\w/]+?)/DGPRIS", "nroRes").group(1).strip()
    d["nroExp"]     = buscar(r"Expte\.\s*N[ºo°]\s*(.+?),\s*por el que", "nroExp").group(1).strip()
    d["paciente"]   = buscar(r"paciente;\s*(.+?)\s+y\s+CONSIDERANDO", "paciente").group(1).strip()
    d["firmasCons"] = buscar(r"Firmas Comerciales;\s*(.+?)\.-", "firmasCons").group(1).strip()
    d["firmaGan"]   = buscar(r"a favor de la firma\s+(.+?)\s+que se detalla", "firmaGan").group(1).strip()
    d["cantidad"]   = int(buscar(r"Aud[íi]fonos\s+(\d+)\s+\$", "cantidad").group(1))
    imp = buscar(r"por el importe de\s*\$\s*([\d.]+),\d{2}\s*\((.+?)\)", "importe")
    d["importeLetras"] = imp.group(2).strip()
    d["total"]      = int(imp.group(1).replace(".", ""))
    if d["cantidad"] <= 0:
        raise ValueError("Cantidad inválida.")
    d["unitario"]   = d["total"] // d["cantidad"]
    return d


# ----------------------------- PROCESO --------------------------------------
def procesar(pdfs):
    resultados, errores = [], []
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for item in pdfs:
            nombre = (item.get("nombre") or "").strip() or "resolucion.pdf"
            try:
                b = base64.b64decode(item["base64"])
                d = extraer_datos(b)
                pdf_out = construir_resolucion(d, mostrar_firma=False)
                apellido = d["paciente"].split(",")[0].strip().replace(" ", "_")
                fname = "Resolucion_%s_%s.pdf" % (d["nroRes"], apellido or "SIN_NOMBRE")
                zf.writestr(fname, pdf_out)
                resultados.append({
                    "nombre": nombre, "archivo": fname,
                    "nroRes": d["nroRes"], "paciente": d["paciente"],
                    "cantidad": d["cantidad"], "unitario": d["unitario"], "total": d["total"],
                })
            except Exception as ex:
                errores.append({"nombre": nombre, "error": str(ex)})
    return buf.getvalue(), resultados, errores


# ----------------------------- HANDLER --------------------------------------
class handler(BaseHTTPRequestHandler):
    def _cors(self):
        self.send_header("Access-Control-Allow-Origin", "*")
        self.send_header("Access-Control-Allow-Methods", "POST, OPTIONS")
        self.send_header("Access-Control-Allow-Headers", "Content-Type")

    def _json(self, code, obj):
        self.send_response(code)
        self._cors()
        self.send_header("Content-Type", "application/json")
        self.end_headers()
        self.wfile.write(json.dumps(obj).encode("utf-8"))

    def do_OPTIONS(self):
        self.send_response(204)
        self._cors()
        self.end_headers()

    def do_POST(self):
        try:
            largo = int(self.headers.get("Content-Length", 0))
            data = json.loads(self.rfile.read(largo).decode("utf-8"))
            pdfs = data.get("pdfs")
            if not pdfs or not isinstance(pdfs, list):
                return self._json(400, {"ok": False, "error": "Falta la lista 'pdfs'."})

            zip_bytes, resultados, errores = procesar(pdfs)
            if not resultados:
                return self._json(422, {"ok": False, "error": "No se pudo procesar ningún PDF.", "errores": errores})

            return self._json(200, {
                "ok": True,
                "zip_base64": base64.b64encode(zip_bytes).decode("ascii"),
                "resultados": resultados,
                "errores": errores,
            })
        except Exception as ex:
            return self._json(500, {"ok": False, "error": str(ex)})
